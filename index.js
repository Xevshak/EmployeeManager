...
function init() {
  const logoText = logo({ name: "Manage Your Employees" }).render();
      ...
      prompt([
        {
          type: "list",
          name: "managerId",
          message: "Who do you ant to see direct reports for?",
  ...
  db.findAllEmployees()
    .then(([rows]) => {
      ...
      prompt([
        ...
        .then(() => console.log("Removed them from the database"))
        .then(() => loadMainPrompts())
    })
}
              ...
              prompt([
                {
                  ...
                  name: "managerId",
                  message:
                    "Who do you want to set as manager for the selected employee?",
                  choices: managerChoices
                }
              ])
  ...
  db.findAllRoles()
    .then(([rows]) => {
      ...
      prompt([
        ...
        .then(() => console.log("Removed from the database"))
        .then(() => loadMainPrompts())
    })
}
